...
namespace FirstGearGames.Mirrors.Assets.FlexNetworkTransforms
{
    ...
    public abstract class FlexNetworkTransformBase : NetworkBehaviour
    {
        ...
        /// <summary>
        /// Percentage of new movement rates to use when transform is speeding up. Higher values will cause the transform to achieve the higher speed faster. Use lower values if you would prefer a more gradual increase over speed changes. Default value is 1f.
        /// </summary>
        [Tooltip("Percentage of new movement rates to use when transform is speeding up. Higher values will cause the transform to achieve the higher speed faster. Use lower values if you would prefer a more gradual increase over speed changes. Default value is 1f.")]
        [Range(0.01f, 1f)]
        [SerializeField]
        private float _accelerateEasing = 1f;
        /// <summary>
        /// Percentage of new movement rates to use when transform is slowing down. Higher values will cause the transform to slow down more rapidly. When using faster synchronization intervals, such as fixed update, this is generally best left close to 1f. When using longer synchronization intervals, such as 100ms, values closer to 0.2f may show better results. Default value is 1f.
        /// </summary>
        [Tooltip("Percentage of new movement rates to use when transform is slowing down. Higher values will cause the transform to slow down more rapidly. When using faster synchronization intervals, such as fixed update, this is generally best left close to 1f. When using longer synchronization intervals, such as 100ms, values closer to 0.2f may show better results. Default value is 1f.")]
        [Range(0.01f, 1f)]
        [SerializeField]
        private float _decelerateEasing = 1f;
        /// <summary>
        ...
        public void ServerDataReceived(ref TransformData data)
        {
            ...
            * to values received from the client. */
            ...
            else
            {
                ...
                moveRates = SetMoveRates(ref data, ref _targetData);
                Snap(ref data, false);
        ...
        /// </summary>
        /// <param name="data"></param>
        private MoveRateData SetMoveRates(ref TransformData data, ref TargetData pastTargetData)
        {
            MoveRateData moveRates = new MoveRateData();
            ...
            //Can only use easing when past data is set.
            if (pastTargetData.IsSet)
            {
                /* If a settled packet use whichever rate is faster. This is to ensure that if extrapolation
                * is overshot then the new move rate will likely be faster, and we want
                * to rebound to proper location faster. However, if it didn't overshoot then
                * new move rates will likely be slower as the transforms current position vs
                * settled goal is likely very close, resulting in slower move rate. For consistency
                * we will use the faster of the two. */
                /* Realistically if one is off they're all off, but position will be the most noticeable. Because of this
                 * only compare position. */
                if (EnumContains.SyncPropertiesContains((SyncProperties)data.SyncProperties, SyncProperties.Settled))
                {
                    //If past position move rate is faster then use past move rates.
                    if (pastTargetData.MoveRates.Position > moveRates.Position)
                        moveRates = pastTargetData.MoveRates;
                }
                //Doesn't have settled.
                else
                {
                    //Can only calculate against new speed if past wasn't settled.
                    if (!EnumContains.SyncPropertiesContains((SyncProperties)pastTargetData.GoalData.SyncProperties, SyncProperties.Settled))
                    {
                        /* When changing to a slower moverate and past moverates are known then ease
                         * into the new move rate. Since smoothing is based on current position/distance/interval this
                         * will help maintain steady speed when coming to a sudden stop such as hitting a wall. For example,
                         * if the unit is moving 1m over 100ms then hits a wall, resulting in the next distance being only 0.5m,
                         * ordinarily it would move that 0.5m over 100ms. This results in an unexpected slowing on the movement.
                         * To help compensate when new speed is slower we use a portion of the old speed to keep momentum. Downside
                         * is that this may overshoot if the speed is very rapidly going from fast to slow every other frame; but this
                         * scenario is unlikely so instead I'm going to favor keeping momentum. */
                        //Only compare against position speed, as all the speeds will likely fall in line but position will be most noticeable.
                        bool nextIsHigher = (moveRates.Position > pastTargetData.MoveRates.Position);
                        if (nextIsHigher && _accelerateEasing < 1f)
                            moveRates.LerpFrom(pastTargetData.MoveRates, _accelerateEasing);
                        else if (!nextIsHigher && _decelerateEasing < 1f)
                            moveRates.LerpFrom(pastTargetData.MoveRates, _decelerateEasing);
                    }
                }
            }
